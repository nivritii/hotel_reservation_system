...
public interface ReservationRepository extends JpaRepository<Reservation, Integer> {
    ...
    List<Reservation> find(Integer roomTypeId, LocalDate start, LocalDate end);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query(value = "SELECT r from Reservation r WHERE r.id = ?1 AND r.cancelled = ?2")
    Optional<Reservation> findByIdAndCancelledForUpdate(Integer id, Boolean cancelled);

    Optional<Reservation> findByIdAndCancelled(Integer id, Boolean cancelled);
}
